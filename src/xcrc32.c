 ...
 *  PKZIP, and ZMODEM.  The crc32_buf() can also be detached and
 *  used independently.
 *
...
/*----------------------------------------------------------------------------*\
 *  NAME:
 *     crc32_file() - compute CRC-32 value for a file
 *  DESCRIPTION:
...
#define CRC_BUFFER_SIZE  8192
    ...
    *outcrc32 = 0;
    while (1) {
        buf_len = fread( buf, 1, CRC_BUFFER_SIZE, file );
        if (buf_len == 0) {
            if (ferror(file)) {
        ...
        *outcrc32 = crc32_buf( *outcrc32, buf, buf_len );
    }
    return( 0 );
...
/*----------------------------------------------------------------------------*\
 *  NAME:
 *     crc32_buf() - computes the CRC-32 value of a memory buffer
 *  DESCRIPTION:
...
void test_xcrc32()
{
  ...
  unsigned long crc32 = crc32_buf(0, buf, strlen(buf));
  printf("0x%lx\n", crc32);
}
#endif
