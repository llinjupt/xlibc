...
/*
 * LLi re-encapsulate it for net-walker project 2010/12/24
 * Merry Christmas day!  lli_njupt@163.com
 */
...
void SHA1Final(unsigned char digest[SHA1_MAC_LEN], SHA1_CTX* context)
{
  uint32_t i;
...
/* Compute SHA1 message digest for LEN bytes beginning at BUFFER.  The
   ...
   digest.  */
void *sha1_buffer(const char *buffer, size_t len, unsigned char resblock[SHA1_MAC_LEN])
{
  SHA1_CTX ctx;
  ...
  SHA1Final(resblock, &ctx);
  
  return resblock;
}

int sha1_stream(FILE *stream, unsigned char resblock[SHA1_MAC_LEN])
{
  SHA1_CTX ctx;
  size_t sum;
  ...
  while (1)
  {
    ...
    SHA1Update(&ctx, buffer, BLOCKSIZE);
  }
...
/* 
 * Note: this fun is only for binary file. For DOS/UNIX test file
 * need to do fopen(file, "r").
 */
void *xsha1file(const char *file, unsigned char digest[SHA1_DIGEST_SIZE])
{
  FILE *fd = fopen(file, "rb");  /* only for binary file */
...
{
  return sha1_buffer(buffer, len, digest);
}
