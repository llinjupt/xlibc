...
{
  /* all timeout events constructed by a list */
  struct dl_list list; 
...
static int eloop_sock_queue(int sock, eloop_event_type type)
{
  ...
  {
    printf("%s: epoll_ctl(ADD) for fd=%d failed: %s\n",
                                     ...
                                     void *user_data)
{
  ...
  if(eloop.count + 1 > eloop.epoll_max_event_num) 
  {
    ...
    if(temp_events == NULL) 
    {
      printf("%s: malloc for epoll failed: %s\n",
             __func__, strerror(errno));
      return -1;
    }
                                         ...
                                         int sock)
{
  ...
  {
    printf("%s: epoll_ctl(DEL) for fd=%d failed: %s\n",
           __func__, sock, strerror(errno));
    return;
  }
...
static void eloop_sock_table_destroy(struct eloop_sock_table *table)
{
  ...
  for(; i < table->count && table->table; i++) 
  {
...
static void test_signal(int sig, void *signal_ctx)
{
  printf("test_signal:%d\n", sig);
}

static void test_term_signal(int sig, void *signal_ctx)
{
  printf("Signal %d received - terminating\n", sig);
  eloop_terminate();
}

#define L2AGNET_PIPE "/tmp/l2agent"
int l2agent_pipe_create()
{
  int fd = 0;

  unlink(L2AGNET_PIPE);
  if(mkfifo(L2AGNET_PIPE, 0644) < 0)
  {
    printf("l2agent_pipe_create mkfifo\n");
    return -1;
  }

  fd = open(L2AGNET_PIPE, O_RDWR);
  if(fd < 0)
  {
    perror("open");
    return -1;
...
void test_eloop()
{
  ...
  eloop_register_signal(SIGUSR1, test_signal, NULL);
  eloop_register_signal_terminate(test_term_signal, NULL);

  eloop_register_timeout(1, 0, test_send, NULL, NULL);  
  eloop_register_timeout(5, 0, test_send, NULL, NULL);

  if(eloop_register_read_sock(fd, handle_read, NULL, NULL))
  {
